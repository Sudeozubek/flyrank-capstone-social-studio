/**
 * Wk5 — Caption composer.
 * Pure, unit-testable: (post, platform) -> tailored caption assembled from
 * shared voice + platform overrides. X and Instagram diverge in structure,
 * tone, length and hashtag count — they are not truncations of one another.
 */
...
/** Trim to a limit on a word boundary, never mid-word, never mid-hashtag. */
...
export function composeCaption(post: BlogPost, platform: Platform): string {
...
export function composeAllCaptions(post: BlogPost, platforms: readonly Platform[]) {
  return platforms.map((platform) => ({ platform, caption: composeCaption(post, platform) }));
}
